...
class BranchSeparablesStem(nn.Module):
  ...
  def __init__(self, in_channels, out_channels, kernel_size, stride,
    ...
    self.separable_1 = SeparableConv2d(in_channels, out_channels,
                                       kernel_size, stride, padding,
                                       bias=bias)
    self.bn_sep_1 = nn.BatchNorm2d(out_channels, eps=0.001, momentum=0.1,
                                   affine=True)
    self.relu1 = nn.ReLU()
...
class CellStem0(nn.Module):
  ...
  def __init__(self):
    ...
    self.conv_1x1.add_module('bn', nn.BatchNorm2d(
        42, eps=0.001, momentum=0.1, affine=True))
  ...
  def forward(self, x):
    ...
    x_out = torch.cat([x_comb_iter_1, x_comb_iter_2,
...
class FirstCell(nn.Module):
  ...
  def __init__(self, in_channels_left, out_channels_left,
    ...
    self.relu = nn.ReLU()
    self.path_1 = nn.Sequential()
    self.path_1.add_module('avgpool', nn.AvgPool2d(
        1, stride=2, count_include_pad=False))
    self.path_1.add_module('conv', nn.Conv2d(
        in_channels_left, out_channels_left, 1, stride=1, bias=False))
    self.path_2 = nn.ModuleList()
    self.path_2.add_module('pad', nn.ZeroPad2d((0, 1, 0, 1)))
    self.path_2.add_module('avgpool', nn.AvgPool2d(
        1, stride=2, count_include_pad=False))
    self.path_2.add_module('conv', nn.Conv2d(
        in_channels_left, out_channels_left, 1, stride=1, bias=False))
  ...
  def forward(self, x, x_prev):
    ...
    x_out = torch.cat([x_left, x_comb_iter_0,
                       x_comb_iter_1, x_comb_iter_2,
                       x_comb_iter_3, x_comb_iter_4], 1)
    return x_out
...
class NormalCell(nn.Module):
  ...
  def __init__(self, in_channels_left, out_channels_left,
    ...
    self.conv_prev_1x1.add_module('conv', nn.Conv2d(
        in_channels_left, out_channels_left, 1, stride=1, bias=False))
...
class NASNetALarge(nn.Module):
  ...
  def __init__(self, num_classes=1001):
    ...
    self.conv0.add_module('bn', nn.BatchNorm2d(
        96, eps=0.001, momentum=0.1, affine=True))
